...
object JvmVersionDetector {
	...
	fun detect(path: Path): String {
		...
		path.resolve("jre/lib/rt.jar").toFile().takeIf {
			it.isFile
		}?.let { rtFile ->
		...
		path.resolve("bin").getExecutable("java").toFile().takeIf {
			it.canExecute()
		}?.let { javaExec ->
		...
		throw IllegalArgumentException("version not detected from path: $path")
	}
}